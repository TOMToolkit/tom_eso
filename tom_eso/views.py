...
def folders_for_observing_run(request):
    ...
    p2_tool_url = ESOFacility().get_p2_tool_url(observing_run_id)
    # logger.debug(f'p2_tool_url: {p2_tool_url}')

    # logger.debug(f'**** htmx info in request.GET: {request.htmx}')
    # logger.debug(f'**** htmx.current_url: {request.htmx.current_url}')
    # logger.debug(f'**** htmx.current_url_abs_path: {request.htmx.current_url_abs_path}')
    # logger.debug(f'**** htmx.prompt: {request.htmx.prompt}')
    # logger.debug(f'**** htmx.target: {request.htmx.target}')
    # logger.debug(f'**** htmx.trigger: {request.htmx.trigger}')
    # logger.debug(f'**** htmx.trigger_name: {request.htmx.trigger_name}')
    # logger.debug(f'**** htmx.triggering_event: {request.htmx.triggering_event}')
    ...
    # logger.debug(f'**** folder_name_choices for obs_run({observing_run_id}): {folder_name_choices}')
...
def show_observation_block(request):
    ...
    try:
        ...
        raise e  # re-raise the exception
    # logger.debug(f'***** observation_block_id: {observation_block_id}')
